...
template<typename T>
std::vector<std::complex<T>> ferrari(fourth_degree_polynomial<T> P)
{
	std::vector<T> coefs = P.get_coefs();
	...
	if (q.real() == 0)
	{
		...
		std::complex<float> D1 = p * p - c_two * c_two * r;
		std::complex<float> sqrt_d_one = sqrt(D1);
		roots[0] = -sqrt((-p + sqrt(sqrt_d_one)) * c_half) - fourth_coef_three;
		roots[1] = -sqrt((-p - sqrt(sqrt_d_one)) * c_half) - fourth_coef_three;
		roots[2] = sqrt((-p + sqrt(sqrt_d_one)) * c_half) - fourth_coef_three;
		roots[3] = sqrt((-p - sqrt(sqrt_d_one)) * c_half) - fourth_coef_three;
