/*
--------------------------------------------------
https://www.researchgate.net/publication/268992597_Polynomial_transformations_of_Tschirnhaus_Bring_and_Jarrard
--------------------------------------------------
...
{
    complex<double> one_third = { 1.0 / 3.0, 0 };
    complex<double> one_twentyseventh = one_third * one_third * one_third;
...
vector<complex<T>> Tschirnhaus_transformation(vector<complex<T>> F)
{
    ...
    complex<double> one_third = { 1.0 / 3.0, 0 };
    complex<double> two_third = one_third * complex < double>{2, 0};
    complex<double> one_twentyseventh = one_third * one_third * one_third;
    ...
    for (int i = 0; i <= 2; i++)    //вычисление шести корней 
    {
        ...
        complex<double> return_to_source = -F[0] * one_third;
        six_roots.push_back(main_roots1[0] + return_to_source);
        six_roots.push_back(main_roots1[1] + return_to_source);
    }
    ...
    for (int j = 0; j <= 5; j++)    //отбрасывание посторонних корней
    {
        ...
        complex<double> result = x_cubic + b0 * x_square + c0 * value + d0;
        double re_root = abs(real(result));
        double im_root = abs(imag(result));
